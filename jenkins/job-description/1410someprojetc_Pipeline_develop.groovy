...
def job_config = [
        job: [
                name: "1410someprojetc_Pipeline_develop"
        ],
        git: [
                branch: "develop"
        ]
...
def customLib = library identifier: '1410someprojetc_JSL@develop', retriever: modernSCM(
  [\$class: 'GitSCMSource',
   remote: 'https://github.developer.allianz.io/jarro06/1410someprojetc_lib.git',
   credentialsId: 'git-token-credentials']) 
...
def jslCustom     = customLib.de.allianz.1410someprojetc.new()
    ...
    stages {
        ...
        stage('Checkout') {
            steps {
                echo "checkout"
                script {
                    jslGit.checkout( config, "JEQP", "1410someprojetc_Pipeline_develop", "develop")
                }
            }    
        }
        stage('Build') {
